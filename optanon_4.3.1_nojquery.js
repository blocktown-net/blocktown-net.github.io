...
(function () {
    ...
    function onLoadButtonFocus() {
        if($('.optanon-alert-box-button-container .cookie-settings-button').length > 0) {
            $('.optanon-alert-box-button-container .cookie-settings-button').focus();
        } else if ($('.optanon-alert-box-button-container .accept-cookies-button').length > 0) {
            $('.optanon-alert-box-button-container .accept-cookies-button').focus();
        } else if ($('.optanon-alert-box-button-container .banner-close-button').length > 0) {
            $('.optanon-alert-box-button-container .banner-close-button').focus();
        } 
    }
    ...
    function substitutePlainTextScriptTags() {
        $('script').filter(function () {
            ...
            var groupIds = $(this).attr('class').toLowerCase().split('optanon-category-')[1].split('-');
    ...
    function insertConsentNoticeHtml() {
        ...
        $('body').prepend('<div id="optanon" class="[[OptanonStyle]]"></div>');
        ...
        for (i = 0; i < json.Groups.length; i += 1) {
            ...
            if (safeGroupName(group) == optanonAboutCookiesGroupName || (isTopLevelGroup(group) && isValidConsentNoticeGroup(group))) {
                ...
                menuItem = $('<li class="menu-item-necessary ' + (groupIsAboutCookies || groupIsActive ? 'menu-item-on' : 'menu-item-off') +
                ...
                $('#optanon #optanon-menu').append(menuItem);
            }
        }

        moreInfo = $('<li class="menu-item-moreinfo menu-item-off" title="' + json.AboutText + '">' +
        ...
        $('#optanon #optanon-popup-body').append('<div id="optanon-popup-body-right">' +
        ...
        $('#optanon #optanon-popup-wrapper').append('<div id="optanon-popup-bottom"> <a href="https://onetrust.com/poweredbyonetrust" target="_blank">' +
    ...
    function getSubgroupToggle(json, group) {
        ...
        var $toggle = $('<span class="optanon-subgroup-fieldset fieldset">' + p + '</span><input type="checkbox" value="check" id="' + toggleId +
    ...
    function InitializeVendorList() {
        ...
        if (checkIscenterTile()) {
            $('#optanon-popup-body').append('<div id="optanon-vendor-consent-back"><button class="vendor-consent-back-link" aria-label="Back">Back</button></div>');
            $('#optanon-popup-body').append('<div class="optanon-vendor-center-tile">' +
                '<div id="optanon-vendor-consent-list">' +
                '<div></div>');
        } else {
            $('#optanon-popup-body-left').append('<div id="optanon-vendor-consent-back"><button class="vendor-consent-back-link" aria-label="Back">Back</button></div>');
            $('#optanon-popup-body-right').append('<div id="optanon-vendor-consent-list"></div>');
        }
       
        if(vendors && vendors.length>0){
            for (i = 0; i < vendors.length; i ++) {
                vendors[i].policyUrl = getValidUrl(vendors[i].policyUrl);
                $("#optanon-vendor-consent-list").append('<div class="vendor-item">' +
        ...
        if (checkIscenterTile()) {
            $('#optanon-popup-body .optanon-vendor-center-tile').prepend('<div class="vendor-list-title"><div class="vendor-consent-title">Vendor Consent </div>' +
            ...
            $('#optanon #optanon-popup-body-right .vendor-header-container').append('<div class="optanon-vendor-list-allow-all">' +
            getVenderListAllowAllToggle() +
            '</div>');
    ...
    function setVendorStatus() {
        ...
        if (oneTrustIABConsent.vendors && vendorCheckBoxList && vendorCheckBoxList.length > 0) {
            for (var i = 0; i < vendorCheckBoxList.length; i++) {
                var isVendorActive = $.inArray((vendorCheckBoxList[i].id + ':true'), oneTrustIABConsent.vendors) != -1;
    ...
    function insertCenterTileConsentNoticeHtml() {
        ...
        $('body').prepend('<div id="optanon" class="modern"></div>');
        var preferenceCenterDataHTML = '<div id="optanon-popup-bg"></div>' +
            '<div id="optanon-popup-wrapper" role="dialog" aria-modal="true" tabindex="-1">' +
            '<div id="optanon-popup-top">';
        ...
        for (i = 0; i < json.Groups.length; i += 1) {
            ...
            if (groupName == optanonAboutCookiesGroupName || (isTopLevelGroup(group) && isValidConsentNoticeGroup(group))) {
                ...
                groupMenuItem = $('<div id="optanon-popup-body-content" class="menu-item-necessary ' + 
        ...
        $('#optanon #optanon-popup-wrapper').append('<div id="optanon-popup-bottom">' +
            '<div class="optanon-bottom-spacer"></div>' +
            moreGroupInfo +
    ...
    function insertShowSettingsButtonsHtml() {
        $('.optanon-show-settings').wrap('<div class="optanon-show-settings-popup-wrapper">').wrap('<div class="optanon-show-settings-button">').wrap('<div class="optanon-show-settings-middle">');
        $('.optanon-show-settings-middle').before('<div class="optanon-show-settings-left"></div>');
        $('.optanon-show-settings-middle').after('<div class="optanon-show-settings-right"></div>');
    ...
    function insertCookiePolicyHtml() {
        ...
        for (i = 0; i < json.Groups.length; i += 1) {
            ...
            if (isTopLevelGroup(group) && isValidConsentNoticeGroup(group)) {
                //Insert group html
                groupHtml = $('<div class="optanon-cookie-policy-group"></div>');
                ...
                if (subGroups.length > 0) {
                    ...
                    subgroupTableHtml = $('<table class="optanon-cookie-policy-subgroup-table"></table>');
                    ...
                    for (j = 0; j < subGroups.length; j += 1) {
                        subgroupHtml = $('<tr class="optanon-cookie-policy-subgroup"></tr>');
    ...
    function getGroupCookiesHtml(group) {
        var json = optanonData(),
            groupHtml = $('<div class="optanon-cookie-list"></div>'),
            j,
            k,
            l,
            subGroups = getGroupSubGroups(group),
        ...
        if (group.Cookies && group.Cookies.length > 0) {
            //Insert group cookies html
            $group = $('<p class="optanon-group-cookies-list"></p>');
        ...
        if (subGroups && subGroups.length > 0) {
            for (j = 0; j < subGroups.length; j += 1) {
                $subgroup = $('<p class="optanon-subgroup-cookies-list ' + (!optanonShowSubGroupCookies ? 'subgroup-cookies-list' : '') + '"></p>');
                ...
                //BEHAVIOUR_END
                var $subgroupCookies = $('<div class="optanon-subgroup-cookies"></div>');
            ...
            if(isCenterTile) {
                ...
                Array.prototype.forEach.call(document.querySelector('#optanon-menu').children, function (el, idx) {
                    ...
                    Array.prototype.forEach.call(document.querySelectorAll('.optanon-status-checkbox'), function (domEl, idx) {
                        if($(domEl).data('group')) {
                            if (domEl.checked) {
                                toggleGroupStatusOn(json, $(domEl).data('group'), domEl);
                            } 
                        }
                    });
