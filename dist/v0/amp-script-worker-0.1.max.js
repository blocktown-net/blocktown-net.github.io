...
var WorkerThread = (function (exports) {
    ...
    /* Initializing */
    ;
    var set = newPhase => phase = newPhase;
    ...
    function transfer$2(document, mutation) {
      ...
      /* allowTransfer */
        ...
        Promise.resolve().then(_ => {
          if (pending) {
            var nodes = new Uint16Array(consume$1().reduce((acc, node) => acc.concat(node[8
            /* creationFormat */
            ]), [])).buffer;
    ...
    var propagate = (node, property, value) => {
      node[property] = value;
    ...
    */
    ...
    class ParentNode extends Node {
      ...
      querySelector(selector) {
        var matches = querySelectorAll(this, selector);
    ...
    var hyphenateKey = key => toLower(key.replace(/(webkit|ms|moz|khtml)/g, '-$1').replace(/([a-zA-Z])(?=[A-Z])/g, '$1-'));

    var appendKeys = keys => {
      var keysToAppend = keys.filter(key => isNaN(key) && !CSSStyleDeclaration.prototype.hasOwnProperty(key));
    ...
    var registerSubclass = function registerSubclass(localName, subclass, namespace) {
      if (namespace === void 0) {
        namespace = HTML_NAMESPACE;
      }

      return NS_NAME_TO_CLASS[namespace + ":" + localName] = subclass;
    };
    ...
    class Element extends ParentNode {
      ...
      get outerHTML() {
        ...
        var start = "<" + [tag, toString(this.attributes)].join(' ').trim() + ">";
     ...
     */
    var _a$5;
    var appendGlobalEventProperties = keys => {
      var keysToAppend = keys.filter(key => !HTMLElement.prototype.hasOwnProperty(key));
    ...
    */
    ...
    class CanvasRenderingContext2DShim {
      ...
      delegateFunc(name, args) {
        var returnValue = this.implementation[name](...args);
    ...
    */
    ...
    class HTMLInputElement extends HTMLElement {
      ...
      stringToDate(str) {
        ...
        var [y, m, d] = components; // Month is 0-index.
    ...
    var insertBeforeElementsWithTagName = (parent, element, tagNames) => {
      var insertBeforeElement = matchChildElement(parent, element => !tagNames.includes(element.tagName));
    ...
    function propagate$1(global) {
      ...
      document.addGlobalEventListener('message', (_ref) => {
        var {
          data
        } = _ref;
    ...
    class Document extends Element {
      ...
      /* hydrateNode */
        ...
        /* nodeType */
          ...
          default:
            ...
            var _constructor = NS_NAME_TO_CLASS[namespaceURI + ":" + localName] || HTMLElement;
      ...
      createElementNS(namespaceURI, localName) {
        var constructor = NS_NAME_TO_CLASS[namespaceURI + ":" + localName] || HTMLElement;
    ...
    function initialize(document, strings, hydrateableNode, cssKeys, globalEventHandlerKeys, _ref, localStorageData, sessionStorageData) {
      var [innerWidth, innerHeight] = _ref;
      appendKeys(cssKeys);
    ...
    var noop = () => void 0; // WorkerDOM.Document.defaultView ends up being the window object.
    ...
    (function (global) {
       ...
       */
      var deleteUnsafe = (object, property) => {
